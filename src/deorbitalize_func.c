...
void xc_evaluate_func(const xc_func_type *func, int order,
                      const xc_input_variables *in, xc_output_variables *out)
{
  /* Evaluate the functional */
  switch(func->info->family){
  case XC_FAMILY_LDA:
    xc_evaluate_lda(func, order, in, out);
    break;
  case XC_FAMILY_GGA:
    xc_evaluate_gga(func, order, in, out);
    break;
  case XC_FAMILY_MGGA:
    xc_evaluate_mgga(func, order, in, out);
    break;
  case XC_FAMILY_HGGA:
    xc_evaluate_hgga(func, order, in, out);
    break;
  }
}
