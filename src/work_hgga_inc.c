...
#endif
{
  ...
  FUNC(ORDER_TXT, SPIN_TXT)(p, ip, my_rho, my_sigma, &INP_VAR(lapl, ip, 0),
                            my_tau, &INP_VAR(exx, ip, 0), out);
  ...
  if(out->vrho != NULL){
    ...
    for(ii=0; ii < dim->vexx; ii++)
      is_OK = is_OK && isfinite(OUT_VAR(vexx, ip, ii));
  }
    ...
    if(p->nspin == XC_UNPOLARIZED){
      ...
      if(p->info->flags & (XC_FLAGS_NEEDS_LAPLACIAN | XC_FLAGS_NEEDS_TAU))
        printf("%le 0.0 %le 0.0 0.0 %le 0.0 %le 0.0 %le 0.0\n",
               INP_VAR(rho, ip, 0), INP_VAR(sigma, ip, 0),
               INP_VAR(lapl, ip, 0), INP_VAR(tau, ip, 0), INP_VAR(exx, ip, 0));
      else if(p->info->flags & XC_FLAGS_NEEDS_LAPLACIAN)
        printf("%le 0.0 %le 0.0 0.0 %le 0.0 0.0 0.0 %le 0.0\n",
               INP_VAR(rho, ip, 0), INP_VAR(sigma, ip, 0),
               INP_VAR(lapl, ip, 0), INP_VAR(exx, ip, 0));
      else
        printf("%le 0.0 %le 0.0 0.0 0.0 0.0 %le 0.0 %le 0.0\n",
               INP_VAR(rho, ip, 0), INP_VAR(sigma, ip, 0),
               INP_VAR(tau, ip, 0), INP_VAR(exx, ip, 0));
    }else{
      ...
      if(p->info->flags & (XC_FLAGS_NEEDS_LAPLACIAN | XC_FLAGS_NEEDS_TAU))
        printf("%le %le %le %le %le %le %le %le %le %le %le\n",
               ...
               INP_VAR(exx, ip, 0), INP_VAR(exx, ip, 1)
               );
      else if(p->info->flags & XC_FLAGS_NEEDS_LAPLACIAN)
