...
rscan_alpha0 := (rs, x, t) -> (np53(rs)*(t - x^2/8))/(np53(rs)*K_FACTOR_C + 2^(2/3)*params_a_taur):

(* Second regularization: alpha -> alpha^3/(alpha^2 + alpha_r) *)
rscan_alpha := (rs, x, t) -> rscan_alpha0(rs, x, t)^3/(rscan_alpha0(rs, x ,t)^2 + params_a_alphar):
...
rscan_f   := (rs, z, x, u, t) -> (scan_h1x(scan_y(x, rscan_alpha(rs, x, t)))*(1 - rscan_f_alpha(rscan_alpha(rs, x, t), rscan_fx))
  + scan_h0x*rscan_f_alpha(rscan_alpha(rs, x, t), rscan_fx))*scan_gx(x):
