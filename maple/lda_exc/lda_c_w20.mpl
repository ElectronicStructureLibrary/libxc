...
(* type: lda_exc *)
(* prefix:
  lda_c_w20_params *params;

  assert(p->params != NULL);
  params = (lda_c_w20_params * )(p->params);
*)
...
w20_b0 := proc(z)
  if (z=0) then
    -0.071100 + log(2)/6 - 3/(4*Pi^2)*Zeta(3)
  elif (z=1) then
    -0.049917 + log(2)/6 - 3/(4*Pi^2)*Zeta(3)
  end if
end proc:
...
(* eq 8 *)
w20_ec := (rs, z) -> -w20_a0(z)/2 * log(1 + w20_DF(rs,z,w20_f0,w20_cx)/rs + w20_E(rs,z)/rs^(3/2) + w20_DF(rs,z,w20_f2,w20_cs)/rs^2) + w20_G(rs,z):

(* eqs 9 and 11 only differ by the c function and f value *)
w20_DF := (rs, z, fval, cfunc) -> exp(-2*w20_b0(z)/w20_a0(z)) - 2 * (1 - exp(-(rs/100)^2))*( (fval - cfunc(z))/w20_a0(z) + 1/2 * exp(-2*w20_b0(z)/w20_a0(z)) ):
