...
if evalb(Polarization = "ferr") then
  b97mv_fpar  := (lda_func, rs, z, xs0, xs1, ts0, ts1) ->
    + lda_func(rs, 1) * b97mv_g(b97mv_gamma_x,  b97mv_wx_ss, b97mv_par_x,  b97mv_par_n, xs0, ts0, 0)
    +     f_pw(rs, 1) * b97mv_g(b97mv_gamma_ss, b97mv_wx_ss, b97mv_par_ss, b97mv_par_n, xs0, ts0, 0):

  b97mv_fos := (rs, z, xs0, xs1, ts0, ts1) -> 0:
else
  b97mv_fpar  := (lda_func, rs, z, xs0, xs1, ts0, ts1) ->
    + lda_stoll_par(lda_func, rs,  z,  1) * b97mv_g(b97mv_gamma_x,  b97mv_wx_ss, b97mv_par_x,  b97mv_par_n, xs0, ts0, 0)
    + lda_stoll_par(f_pw    , rs,  z,  1) * b97mv_g(b97mv_gamma_ss, b97mv_wx_ss, b97mv_par_ss, b97mv_par_n, xs0, ts0, 0)
    + lda_stoll_par(lda_func, rs, -z, -1) * b97mv_g(b97mv_gamma_x,  b97mv_wx_ss, b97mv_par_x,  b97mv_par_n, xs1, ts1, 0)
