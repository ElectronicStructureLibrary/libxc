...
{
  FLOAT mu, alpha, ss, ss2, f0, f1, df0, d2f0;
  ...
  *dfdx  = X2S*2.0*mu*ss*(1.0 - alpha*ss2 - alpha*mu*ss2*ss2)*f0/(f1*f1);

  if(order < 2) return;

  *d2fdx2 = X2S*X2S*2.0*mu*
    (1.0 + 
     ss2*(-3.0*mu - 5.0*alpha + 
	  ss2*(2.0*alpha*alpha - 6.0*alpha*mu + 
	       ss2*(-alpha*mu*mu + 4.0*alpha*alpha*mu + 
		    ss2*(2.0*alpha*alpha*mu*mu)))))*
    f0/(f1*f1*f1);
}
